...
def get_background_music(track_name, duration_ms):
    ...
    # Add some beeps at different frequencies based on the track name
    # This is a simplified mock implementation
    if "nature" in track_name.lower():
        # Nature sounds: gentle beeps
        for i in range(0, duration_ms, 2000):
            beep = AudioSegment.silent(duration=100).low_pass_filter(1000)
            audio = audio.overlay(beep, position=i)
    elif "sci-fi" in track_name.lower():
        # Sci-fi sounds: higher frequency beeps
        for i in range(0, duration_ms, 1500):
            beep = AudioSegment.silent(duration=50).high_pass_filter(2000)
            audio = audio.overlay(beep, position=i)
    elif "suspense" in track_name.lower():
        # Suspenseful sounds: low, drawn-out beeps
        for i in range(0, duration_ms, 3000):
            beep = AudioSegment.silent(duration=200).low_pass_filter(500)
            audio = audio.overlay(beep, position=i)
...
# Step 3: Voice Generation
    ...
    if not st.session_state.parsed_data or not st.session_state.character_voices:
        ...
        st.write("Add ambient sounds or music to enhance your audio story.")
